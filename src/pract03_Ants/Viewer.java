    ...
    public void paint(Graphics g) {
       ...
       g.setColor(Color.GRAY); g.setFont(plain);
        ...
        for (int i=0; i<=idx; i++) {
            Op op=ops[i];
            try {
            if (Class.forName("pract03_Ants.Hi"    ).isInstance(op)) {mat.opHi     (op.a,((Hi)op).p);}
            if (Class.forName("pract03_Ants.Bye"   ).isInstance(op)) {mat.opBye     (op.a);}
            if (Class.forName("pract03_Ants.Turn"  ).isInstance(op)) {mat.opTurn     (op.a,((Turn)op).d);}
            if (Class.forName("pract03_Ants.Go"    ).isInstance(op)) {mat.opGo     (op.a);}
            if (Class.forName("pract03_Ants.ChgDir").isInstance(op)) {mat.opChgDir(op.a,((ChgDir)op).d);}
